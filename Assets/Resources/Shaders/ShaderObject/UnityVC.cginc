...
)
{
	...
	gi.indirect.diffuse -= 0.05 * _IndirectDiffuse.a;
	gi.indirect.specular -= 0.05 * _IndirectDiffuse.a;
	gi.indirect.diffuse += _IndirectDiffuse.rgb * _IndirectDiffuse.a;
	gi.indirect.specular += _IndirectDiffuse.rgb * _IndirectDiffuse.a;

	//UnityLight mainLight = MainLight();
	//half light = max(0, dot(s.normalWorld, mainLight.dir));
	//half3 rampColor = tex2D(_RampTex, half2(1 - light, 0));
	//s.diffColor *= rampColor;
	//s.specColor *= rampColor;
	//gi.indirect.diffuse += rampColor - 0.52;
	//gi.indirect.diffuse = saturate(gi.indirect.diffuse);
	//gi.indirect.specular += rampColor - 0.52;
	//gi.indirect.specular = saturate(gi.indirect.specular);
	...
	// color key test
	half4 c = tex2D(_MainTex, i.tex.xy);
	if ((c.r + c.g + c.b) > 2)
	{
		half2 rampUV = 0;
		rampUV.x += (_Time.g * 1) + ((i.tex.x + i.tex.y) * 32);
		half3 rampColor = tex2D(_RampTex, rampUV) * 2;
		color = lerp(color, rampColor, _RampLevel);
	}

	//color += rampColor - 0.52;
	...
	color *= i.color.rgb * _IntensityVC;
	...
	//color += rampColor - 0.52;
	//color = saturate(color);
	//s.diffColor += rampColor - 0.52;
	//s.specColor += rampColor - 0.52;
	...
	outEmission = half4(color * _EmissionVC, 1);
	//outEmission = half4(color * 0.0, 1);

	//outEmission.rgb = rampColor;
